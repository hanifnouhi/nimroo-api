...
export class AuthService {
  ...
  async validateUser(email: string, pwd: string): Promise<UserResponseDto> {
    ...
    try {
      ...
      this.logger.error(
        ...
        `Error in validating user with ${email} email`,
      );
  ...
  async signup(createUserDto: CreateUserDto): Promise<UserDocument> {
    this.logger.debug(
      `Attempting to signup a user with ${createUserDto.email} email`,
    );
    //must hash the password just in case of local provider
    if (
      !createUserDto.provider ||
      createUserDto.provider === UserProvider.Local
    ) {
  ...
  async login(
    ...
    this.logger.debug(
      `Attempting to login user with ${user.email} email and assign access and refresh tokens`,
    );
    ...
    try {
      this.logger.debug(
        `Attempting to update user refresh token with id: ${user.id}`,
      );
      ...
      this.logger.error(
        { error: error.message, stack: error.stack },
        `Error in updating user refresh token with id: ${user.id}`,
      );
      throw new InternalServerErrorException();
    }
  }
  ...
  async googleLogin(req: any, response: Response): Promise<any> {
    ...
    if (!req.user) {
      this.logger.error(
        { error: 'Google login request does not contain user object' },
        `The returned request from google login doesn't contain user object.`,
      );
    ...
    try {
      ...
      this.logger.debug(
        `Attempting to find user with email: ${req.user.email}`,
      );
      ...
      if (!user) {
        ...
        this.logger.debug(
          `Attempting to update user data with email: ${email}`,
        );
      ...
      this.logger.info(
        `User with email: ${email}, created or updated successfully`,
      );
  ...
  async changePassword(
    ...
    this.logger.debug(
      `Attempting to change user password by user id: ${userId}`,
    );
    changePasswordDto.password = await this.hashPassword(
  ...
  async veryifyUserRefreshToken(
    ...
    this.logger.debug(
      `Attempting to verify user refresh token by user id: ${userId}`,
    );
    try {
      ...
      this.logger.error(
        { error: error.message, stack: error.stack },
        `Error in verifing user with id: ${userId}`,
      );
      if (
      ...
      throw new InternalServerErrorException(
        'Unexpected error while verifying refresh token',
      );
    }
  }
  ...
  async verifyEmail(
    ...
    try {
      ...
      this.logger.error(
        { error },
        `Error in sending verification email to ${email}`,
      );
      return false;
    }
  }
  ...
  async resendVerificationEmail(email: string): Promise<boolean> {
    ...
    if (
      ...
      Date.now() - user.verificationEmailSentAt.getTime() < 1000 * 60 * 60 * 24
    ) {
      throw new BadRequestException(
        'You can only resend the verification email once every 24 hours',
      );
    }
  ...
  async sendPasswordResetEmail(email: string): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(
        { error },
        `Error in sending reset password email to ${email}`,
      );
      return false;
    }
  }
  ...
  async validateVerifyEmailToken(token: string): Promise<boolean> {
    try {
      ...
      this.logger.info(
        `Verify email token is valid with user id: ${payload.userId}`,
      );
  ...
  async validateResetPasswordToken(token: string): Promise<boolean> {
    try {
      ...
      const payload = this.jwtService.verify(
        token,
        this.configService.getOrThrow('JWT_RESET_PASSWORD_SECRET'),
      );
      this.logger.info(
        `Reset password token is valid with user id: ${payload.userId}`,
      );
      return true;
    } catch (error) {
  ...
  async unlinkProvider(
    ...
    this.logger.debug(
      `Attempting to unlink provider from user data with id: ${userId}`,
    );
  ...
  async comparePasswords(password: string, hashedPassword: string) {
    this.logger.debug(
      `Attempting to compare user password with hashed password`,
    );
