...
describe('AuthService - Unit', () => {
  ...
  beforeEach(async () => {
    ...
    configService = {
      getOrThrow: jest.fn((key: string) => {
        ...
        if (key === 'JWT_EMAIL_SECRET' || key === 'JWT_RESET_PASSWORD_SECRET')
  ...
  describe('validateUser', () => {
    it('should return user dto if password matches', async () => {
      ...
      expect((service as any).logger.info).toHaveBeenCalledWith(
        expect.stringContaining('successfully validated'),
      );
    });
    ...
    it('should throw InternalServerErrorException and log error if something fails', async () => {
      ...
      await expect(
        service.validateUser('a@test.com', '123456'),
  ...
  describe('login', () => {
    it('should set cookies and update refresh token', async () => {
      ...
      expect((service as any).logger.info).toHaveBeenCalledWith(
        expect.stringContaining('tokens cookies were set'),
      );
    });
    ...
    it('should redirect if redirect flag is true', async () => {
      ...
      expect((service as any).logger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Redirect to'),
      );
    });
    ...
    it('should throw InternalServerErrorException and log error on failure', async () => {
      ...
      userService.updateRefreshToken.mockRejectedValue(
        new Error('update error'),
      );

      await expect(service.login(user, response)).rejects.toThrow(
        InternalServerErrorException,
      );
  ...
  describe('veryifyUserRefreshToken', () => {
    it('should return user if refresh token matches', async () => {
      ...
      expect((service as any).logger.info).toHaveBeenCalledWith(
        expect.stringContaining('refresh token verified'),
      );
    });
    ...
    it('should throw NotFoundException if user is not found', async () => {
      ...
      await expect(
        service.veryifyUserRefreshToken('any-token', '1'),
  ...
  describe('verifyEmail', () => {
    ...
    it('should call updateVerificationEmailSentAt from user module', async () => {
      jwtService.sign.mockReturnValue(token);
      const updateVerificationEmailSentAtSpy = jest
        .spyOn(userService, 'updateVerificationEmailSentAt')
    ...
    it('should return false if the verification email not sent', async () => {
      ...
      emailService.sendVerificationEmail.mockRejectedValue(
        new Error('Error in sending verification email to test@test.com'),
      );
      const result = await service.verifyEmail(
        userId,
        userEmail,
  ...
  describe('sendPasswordResetEmail', () => {
    ...
    it('should call updatePasswordResetEmailSentAt from user module', async () => {
      ...
      jwtService.sign.mockReturnValue(token);
      const updatePasswordResetEmailSentAtSpy = jest
        .spyOn(userService, 'updatePasswordResetEmailSentAt')
        .mockResolvedValue(undefined);
      await service.sendPasswordResetEmail(userEmail);
    ...
    it('should return false if the verification email not sent', async () => {
      ...
      emailService.sendPasswordResetEmail.mockRejectedValue(
        new Error('Error in sending password reset email to test@test.com'),
      );
      const result = await service.sendPasswordResetEmail(userEmail);
    ...
    it('should throw NotFoundException if user is not found', async () => {
      ...
      await expect(service.sendPasswordResetEmail(userEmail)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('resendVerificationEmail', () => {
    ...
    it('should throw NotFoundException if user is not found', async () => {
      userService.findByEmail.mockResolvedValue(null);
      await expect(service.resendVerificationEmail(userEmail)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('comparePasswords', () => {
    it('should call bcrypt.compare and log debug', async () => {
      ...
      expect((service as any).logger.debug).toHaveBeenCalledWith(
        expect.stringContaining('compare user password'),
      );
    });
  });
  ...
  describe('validateVerifyEmailToken', () => {
    ...
    it('should return false if jwt.verify throws and not call user service update', async () => {
      const spy = jest.spyOn(jwtService, 'verify').mockImplementation(() => {
  ...
  describe('AuthService.unlinkProvider', () => {
    ...
    it('should throw error if user service throws error', async () => {
      userService.unlinkProvider.mockRejectedValue(
        new Error('Error in unlinking provider'),
      );

      await expect(
        service.unlinkProvider(userId, UserProvider.Google),
      ).rejects.toThrow('Error in unlinking provider');
    });
  });
});
