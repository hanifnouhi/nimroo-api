...
export class CacheService {
  ...
  async setCachedValue<T>(key: string, value: T, ttl?: number): Promise<void> {
    this.logger.debug(
      `Attempting to set the cached value with key: ${key} and value: ${value} and ttl: ${ttl}`,
    );
  ...
  async getOrSetCachedValue<T>(
    ...
    this.logger.debug(
      `Attempting to get or set the cache value with key: ${key}`,
    );
    try {
    ...
    try {
      ...
      this.logger.debug(
        `Attempting to run call back function in case of not found in the cache with key: ${key}`,
      );
