...
export class UsageInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
        ...
        const isMembershipDisabled = this.configService.get<string>('DISABLE_MEMBERSHIP_SYSTEM') === 'true';
        ...
        const feature = this.reflector.get<MembershipFeature>(LIMIT_KEY, context.getHandler());
        ...
        if (typeof limit === 'boolean') {
            if (limit === false) {
                throw new ForbiddenException(`Your ${userPlan} plan does not include ${feature}.`);
            }
        ...
        if (typeof limit === 'number') {
            if (limit === 0) {
                throw new ForbiddenException(`Feature ${feature} is not available in your current plan.`);
            }
            ...
            if (currentUsage >= limit) {
                throw new ForbiddenException(`Daily limit of ${limit} reached for ${feature}.`);
            }
            ...
            return next.handle().pipe(
                tap(async () => {
                    ...
                    await this.cacheService.setCachedValue(cacheKey, currentUsage + 1, 86400);
                }),
            );
        }
