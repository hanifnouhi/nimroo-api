...
describe('UsageInterceptor', () => {
  ...
  const createMockContext = (user: any, handler: any): Partial<ExecutionContext> => ({
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: { getOrSetCachedValue: jest.fn(), setCachedValue: jest.fn() },
        },
        {
          provide: UserService,
  ...
  describe('Boolean Permissions', () => {
    it('should throw ForbiddenException if feature is false in plan', async () => {
      ...
      jest.spyOn(reflector, 'get').mockReturnValue(MembershipFeature.CLOUD_SYNC);
      ...
      await expect(interceptor.intercept(context, mockCallHandler))
        .rejects.toThrow(ForbiddenException);
    });

    it('should allow access if feature is true in plan', async () => {
      jest.spyOn(reflector, 'get').mockReturnValue(MembershipFeature.CLOUD_STORAGE);
      const context = createMockContext(
        { userId: '1', membership: MembershipPlan.FREE },
        {}
      ) as ExecutionContext;
  ...
  describe('Numeric Quotas', () => {
    it('should throw ForbiddenException if daily limit is reached', async () => {
      jest.spyOn(reflector, 'get').mockReturnValue(MembershipFeature.CARD_CREATE);
    ...
    it('should allow access and increment usage if under limit', async () => {
        ...
        expect(userService.incrementUsage).toHaveBeenCalledWith('user-123', MembershipFeature.CARD_CREATE);
    ...
    it('should NOT increment usage if the request handler fails', async () => {
        ...
        const failingHandler = {
            handle: jest.fn().mockReturnValue(new Observable(sub => sub.error(new Error('Boom'))))
        };
  ...
  describe('Edge Cases', () => {
    it('should pass through if no feature metadata is found', async () => {
        ...
        const context = createMockContext({ userId: '1' }, {}) as ExecutionContext;
    ...
    it('should use 86400s TTL when getting or setting cache', async () => {
        ...
        const context = createMockContext({ userId, membership: MembershipPlan.FREE }, {}) as ExecutionContext;
        const result = await interceptor.intercept(context, mockCallHandler);
        await lastValueFrom(result);
    ...
    it('should throw ForbiddenException if limit is 0', async () => {
        jest.spyOn(reflector, 'get').mockReturnValue(MembershipFeature.IMAGE_GENERATION);
        
        const context = createMockContext({ userId: '1', membership: MembershipPlan.FREE }, {}) as ExecutionContext;

        await expect(interceptor.intercept(context, mockCallHandler))
        .rejects
        .toThrow(new ForbiddenException(`Feature ${MembershipFeature.IMAGE_GENERATION} is not available in your current plan.`));
