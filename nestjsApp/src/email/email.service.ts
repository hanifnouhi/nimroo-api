...
export class EmailService {
  ...
  async sendVerificationEmail(to: string, token: string): Promise<any> {
    ...
    try {
      //read email template
      this.logger.debug(
        `Attempting to read send verification email template from file system`,
      );
      ...
      this.logger.error(
        { error: err },
        `Error in reading send verification email template file`,
      );
    ...
    try {
      ...
      this.logger.error(
        { error },
        `Error in sending verification email to ${to}`,
      );
      throw error;
    }
  }
  ...
  async sendPasswordResetEmail(to: string, token: string): Promise<any> {
    ...
    try {
      //read email template
      this.logger.debug(
        `Attempting to read reset password email template from file system`,
      );
      const source = fs.readFileSync(templatePath, 'utf8');
      const template = handlebars.compile(source);
      ...
      this.logger.error(
        { error: err },
        `Error in reading reset password email template file`,
      );
      //send a simple html body in case of missed template
      html = `
                <div style="font-family: Arial, sans-serif; padding: 20px;">
    ...
    try {
      ...
      this.logger.error(
        { error },
        `Error in sending password reset email to ${to}`,
      );
      throw error;
    }
  }
