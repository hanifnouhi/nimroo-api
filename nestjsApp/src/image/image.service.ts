...
export class ImageService {
  ...
  constructor(
    ...
    @Inject('ImageGenerateProvider')
    private generateProvider: ImageGenerateProvider,
  ...
  async search(text: string, sourceLang: string): Promise<ImageResultDto[]> {
    this.logger.debug(
      `Attempting to search image for text: ${text} and source language: ${sourceLang}`,
    );
    const imageSearchResult: ImageResultDto[] = [];
    let searchedText = normalizeText(text);

    //if the text is not in english, send request to translate the text to english
    if (sourceLang !== 'en') {
      try {
        const translateResult = await this.translateService.translate(
          searchedText,
          'en',
          sourceLang,
          false,
        ...
        this.logger.error(
          ...
          `Error in translating text: ${text}`,
        );
      }
    }
    ...
    try {
      //search in the cache
      const result = await this.cacheService.getOrSetCachedValue<
        ImageResultDto[]
      >(cacheKey, async () => {
        ...
        try {
          ...
          llmAnalyzeResult = await this.llmService.analyzeText({
            text: searchedText,
          });
        } catch (error) {
          this.logger.error(
            { error: error.message, stack: error.stack },
            `Error in analyzing text: ${searchedText}`,
          );
        ...
        if (
          (llmAnalyzeResult && llmAnalyzeResult.meaningful) ||
          llmServiceFailed
        ) {
          try {
            ...
            if (images && images.length > 0) {
              for (const img of images) {
                imageSearchResult.push({
                  imageUrl: img.url,
                  downloadUrl: img.download,
                });
              }
            }
          } catch (error) {
            this.logger.error(
              { error: error.message, stack: error.stack },
              `Error in searhing image: ${searchedText}`,
            );
            throw error;
          }
        }
      ...
      this.logger.error(
        { error: error.message, stack: error.stack },
        `Error in searching image with text: ${searchedText}`,
      );
  ...
  async generate(text: string): Promise<ImageResultDto[]> {
    ...
    const imageGenerateResult: ImageResultDto[] = [];
    ...
    try {
      ...
      >(cacheKey, async () => {
        try {
          ...
          if (image.imageBuffer && this.containerName) {
            ...
            try {
              const imageUrl = await this.storageService.uploadFile(
                this.containerName,
                Buffer.from(image.imageBuffer),
                fileName,
                'image/jpeg',
              );
              if (imageUrl) {
                imageGenerateResult.push({
                  imageUrl: imageUrl,
          ...
          this.logger.error(
            { error },
            `Error in generating image with text: ${text}`,
          );
          throw error;
        }
      });
