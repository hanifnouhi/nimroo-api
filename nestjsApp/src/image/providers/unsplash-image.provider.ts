...
export class UnsplashImageProvider implements ImageSearchProvider {
  constructor(
    ...
    @InjectPinoLogger(UnsplashImageProvider.name)
    private readonly logger: PinoLogger,
  ) {}

  async search(text: string): Promise<ImageSearchResult[]> {
    this.logger.debug(
      `Attempting to search image with unsplash api for text: ${text}`,
    );
    ...
    try {
      ...
      this.logger.info(
        `Search image sueccessfully done with ${response.data.results.length} images`,
      );
      ...
      this.logger.error(
        ...
        `Error in searching image with text: ${text}`,
      );
      throw error;
    }
  }
