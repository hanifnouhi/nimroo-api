...
export class LlmController {
    ...
    async textData(@Body() textDto: LlmTextDataDto): Promise<LlmTextDataResultDto> {
        this.logger.debug(`Received POST request to /textData with data: ${JSON.stringify(textDto)}`);
        return await this.llmService.textData(textDto.text, textDto.sourceLang, textDto.targetLang);
    }
}
