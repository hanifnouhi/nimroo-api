...
export class LlmService {
     ...
     */
    async textData(text: string, sourceLang: string, targetLang: string): Promise<LlmTextDataResultDto> {
        // Cache key for test data
        const cacheKey = `llm:data:${text}:${sourceLang}:${targetLang}:v1`;
        
        this.logger.debug(`Attempt to get text data for: ${text} in language: ${targetLang}`);
