...
describe('AzureOpenAiProvider', () => {
  ...
  beforeEach(() => {
    ...
    configService = {
      getOrThrow: jest.fn((key: string) => {
        ...
        if (key === 'AZURE_OPEN_AI_ANALYZE_API_VERSION')
          return 'http://nimroo.com';
        if (key === 'AZURE_OPEN_AI_ANALYZE_DEPLOYMENT')
          return 'http://nimroo.com';
  ...
  it('should return parsed JSON when response is valid', async () => {
    mockClient.chat.completions.create.mockResolvedValueOnce({
      choices: [
        { message: { content: '{"meaningful": true, "visualizable": false}' } },
      ],
    });
  ...
  it('should throw error when response is invalid JSON', async () => {
    ...
    await expect(
      azureOpenAiProvider.analyze({ text: 'invalid' }),
  ...
  it('should fallback to undefined if content is undefined', async () => {
    ...
    await expect(
      azureOpenAiProvider.analyze({ text: 'test' }),
