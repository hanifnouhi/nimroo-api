...
const llmTextDataCorrectResult: LlmTextDataResultDto = {
  meaning: 'Fruit comestible, souvent rouge ou verte. Arbre qui produit ce fruit.',
  examples: [
    "Je mange une pomme.",
    "La pomme est rouge.",
...
describe('LlmService', () => {
  ...
  describe('Text data', () => {
    it('should return an object containing meaning, examples, sysnonyms, antonyms', async () => {
      ...
      expect(result).toStrictEqual(expect.objectContaining(llmTextDataCorrectResult));
    });
  
    it('should throw error if undefined returned from llm provider', async () => {
      llmAnalyzeProvider.textData.mockRejectedValue(new Error(`Failed to parse LLM response: ${text}`));
      await expect(service.textData(text, sourceLang, targetLang)).rejects.toThrow(`Failed to parse LLM response: ${text}`);
      expect(mockLlmAnaylyzeProvider.textData).toHaveBeenCalledWith(text, sourceLang, targetLang);
    });
  
    it('should return result from cache if found the record in the cache', async() => {
      (mocks.cacheService!.getOrSetCachedValue as jest.Mock).mockResolvedValue(llmTextDataCorrectResult);
      ...
      expect(mocks.cacheService!.getOrSetCachedValue).toHaveBeenCalledWith(
        expect.stringContaining(`llm:data:${text}:${sourceLang}:${targetLang}:v1`),
        expect.any(Function)
      );
    });
    ...
    it('should call provider and cache result if not found in cache', async() => {
      ...
      expect(result).toBe(llmTextDataCorrectResult);
      expect(llmAnalyzeProvider.textData).toHaveBeenCalledWith(text, sourceLang, targetLang);
      expect(mocks.cacheService!.getOrSetCachedValue).toHaveBeenCalledWith(
        expect.stringContaining(`llm:data:${text}:${sourceLang}:${targetLang}:v1`),
        expect.any(Function)
      );
