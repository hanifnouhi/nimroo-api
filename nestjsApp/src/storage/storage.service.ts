...
export class StorageService {
    ...
    async uploadFile(containerName: string, fileBuffer: Buffer, fileName: string, mimeType: string): Promise<string> {
        ...
        try {
          const result = await this.storageProvider.uploadFile(containerName, fileBuffer, fileName, mimeType);
          this.logger.info({ fileName, containerName, result }, 'File uploaded successfully');
          return result;
        } catch (error) {
    ...
    async deleteFile(containerName: string, fileName: string): Promise<boolean> {
        ...
        try {
            const result = await this.storageProvider.deleteFile(containerName, fileName);
            this.logger.info({ fileName, containerName, result }, 'File deleted successfully');
            return result;
        } catch (error) {
