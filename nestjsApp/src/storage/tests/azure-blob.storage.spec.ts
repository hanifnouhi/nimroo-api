...
describe('Azure Blob Provider Unit Tests', () => {
                ...
                AzureBlobProvider,
                {
                    ...
                    useValue: {
                        get: jest.fn((key: string) => {
                            if (key === 'AZURE_STORAGE_CONNECTION_STRING') return 'mock-connection-string';
                            return null;
                        }),
                    },
                },
    ...
    it.each([
        ...
        undefined,
        ''
    ])('should throw error if connection string is empty', async(returnedValue) => {
    ...
    it('should returns a string', async () => {
        const result = await azureBlobProvider.uploadFile(containerName, mockBuffer, fileName, mimeType);
        expect(typeof result).toBe('string');
    });

    it('should get the container client', async () => {
        await azureBlobProvider.uploadFile(containerName, mockBuffer, fileName, mimeType);
        expect(mockBlobClient.getContainerClient).toHaveBeenCalledWith(containerName);
    });
    ...
    it('should create the blob container if not exist', async() => {
        ...
        expect(mockContainerClient.createIfNotExists).toHaveBeenCalledWith({ access: 'blob' });
    });
