...
export class TranslateService {
  constructor(
    ...
    @InjectPinoLogger(TranslateService.name)
  ...
  async translate(
    ...
    this.logger.debug(
      `Attempting to translate ${text} to the ${lang} language`,
    );
    ...
    try {
      const result =
        await this.cacheService.getOrSetCachedValue<TranslationResult>(
          cacheKey,
          async () => {
            try {
              this.logger.debug(
                `Cache miss for key: ${cacheKey}. Fetching actual translation from provider...`,
              );
              const translationProvider = await this.provider.translate(
                normalizedText,
                targetLang,
              );
              ...
              if (translationProvider.translated) {
                this.logger.info(
                  `${normalizedText} translated successfully to ${translationProvider.translated}`,
                );

                const languageForSpellCheck =
                  sourceLang ||
                  translationProvider.detectedLanguage?.trim().toLowerCase() ||
                  'en';

                // Check spell just if the language is supported and spell check flag is true
                if (
                  SUPPORTED_SPELLCHECK_LANGUAGES.has(languageForSpellCheck) &&
                  spellCheck
                ) {
                  try {
                    this.logger.debug(
                      `Attempting to check the text spell after successful translate: ${normalizedText}, fromLang ${languageForSpellCheck}`,
                    );
                    let correctedText = await this.spellCheckService.correct(
                      normalizedText,
                      languageForSpellCheck,
                    );
                    ...
                    if (correctedText && correctedText !== normalizedText) {
                      ...
                      this.logger.info(
                        `${normalizedText} spell checked successfully to ${correctedText}`,
                      );
                    }
                  } catch (spellCheckError) {
                    this.logger.error(
                      {
                        error: spellCheckError.message,
                        stack: spellCheckError.stack,
                      },
                      'Spell check API error after successful translate within factory.',
                    );
                  }
                }
              }
              this.logger.debug(
                `Prepared result for caching: ${JSON.stringify(translationProvider)}`,
              );
      ...
      this.logger.debug(
        `Translation result for ${text} obtained (from cache or new).`,
      );
      return result;
    } catch (error) {
      ...
      if (spellCheck) {
        this.logger.debug(
          `Attempting to check the text spell after error in translate: ${normalizedText}`,
        );
        correctedTextOnError = await this.spellCheckService.correct(
          normalizedText,
          sourceLang,
        );
        if (correctedTextOnError) {
          ...
          this.logger.info(
            `${normalizedText} spell checked successfully to ${correctedTextOnError}`,
          );
        }
      }
